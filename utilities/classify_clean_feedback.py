...
reduction = FisherFeaturesUncorr(output_dim = 200, labelA = 0, labelB = 1)
# scaling = mdp.nodes.NormalizeNode()
reservoir = Oger.nodes.LeakyReservoirNode(output_dim=300, leak_rate=0.02)
...
pre_flow = mdp.Flow([preprocess, reduction], verbose=1)
...
flow = mdp.Flow([reduction, pca, classify], verbose=1)
