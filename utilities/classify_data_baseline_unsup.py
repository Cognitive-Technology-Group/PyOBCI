...
gf3 = fish_good_features(X_new[:N_train,], y[:N_train], -1, 1, 10)
...
# model = sklearn.cluster.MiniBatchKMeans(n_clusters=1000, reassignment_ratio=0.05)
model = sklearn.cluster.DBSCAN()
# model = sklearn.cluster.AffinityPropagation()
# model = sklearn.cluster.AgglomerativeClustering(n_clusters = 3, memory="/tmp")
...
# rf = KNeighborsClassifier(n_neighbors=3, weights='distance')
rf = RandomForestClassifier(max_depth=11, n_estimators=4, max_features=1)
# rf = AdaBoostClassifier(n_estimators=100) # to try
rf.fit(c2, y2)
print(rf.score(c2, y2))
