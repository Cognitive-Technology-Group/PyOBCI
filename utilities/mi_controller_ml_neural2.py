...
class MIPlotter(object):
    ...
    def plot(self):
        ...
        n_features = len(self.good_indexes)
        
        features = np.zeros( (look_back, n_features) )
        # print(features.shape)
        
        train_data = ClassificationDataSet(n_features, 1, nb_classes=2)
        for i in range(look_back):
            d = data[i:(box_width+i)]
            fff = extract_features(d)
            fff = fff[self.good_indexes]
            train_data.addSample(fff, [1])
#            features[i, ...] = fff[self.good_indexes]

        probs = self.model.activateOnDataset(train_data)
        ...
        for i in range(look_back):
            control = 0.1 * control_arr[i] + 0.8 * control
        ...
        #self.control_s = np.append(self.control_s, control - control_f)
        ...
        #self.control_s = self.control_s[-100:]
        ...
        if control_f > -0.02 and control_f < 0.02:
            self.current = "baseline"
        elif control_f > 0.04:
            self.current = "right"
        elif control_f < -0.04:
        ...
        hlines(0.0, 0, 40, linestyles='dashed')
